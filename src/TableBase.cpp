...
TableBase::TableBase(int X, int Y, int W, int H, const char *l) : Fl_Group(X,Y,W,H,l)
{
	...
	// NOTE: TableBase use frames not boxes or scrolls will not be drawn
	// and you'll get flickering
	box(FL_DOWN_FRAME);
...
void TableBase::get_selection(int& s_top, int& s_left, int& s_bottom, int& s_right)
{
	if (select_col > current_col) {
		s_left = current_col;
		s_right = select_col;
...
void TableBase::set_selection(int s_top, int s_left, int s_bottom, int s_right)
{
	damage_zone(current_row, current_col, select_row, select_col);
	current_col = s_left;
	current_row = s_top;
