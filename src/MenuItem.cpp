...
// This file contains code for implementing MenuItem, and for
...
const MenuItem* MenuItem::next(int n) const {
...
public:
  const MenuItem* menu;
  menutitle(int X, int Y, int W, int H, const MenuItem*);
};
...
menutitle::menutitle(int X, int Y, int W, int H, const MenuItem* L) :
...
menuwindow::menuwindow(const MenuItem* m, int X, int Y, int Wp, int Hp,
		       const MenuItem* picked, const MenuItem* t, 
  ...
  : Fl_Menu_Window(X, Y, Wp, Hp, 0)
{
  ...
  if (selected >= 0)
    ...
    if (Y+h()>scr_y+scr_h && Y-h()>=scr_y) {
      if (Hp>1) 
        // if we know the height of the MenuBase, use it
        Y = Y-Hp-h();
      else if (t)
...
int menuwindow::find_selected(int mx, int my) {
  ...
  if (!itemheight) { // menubar
    int xx = 3; int n = 0;
    const MenuItem* m = menu ? menu->first() : 0;
    for (; ; m = m->next(), n++) {
      if (!m->text) return -1;
...
struct menustate {
  const MenuItem* current_item; // what mouse is pointing at
...
static inline void setitem(const MenuItem* i, int m, int n) {
  p->current_item = i;
  p->menu_number = m;
  p->item_number = n;
}
...
static int forward(int menu) { // go to next item in menu menu if possible
  menustate &pp = *p;
  // MenuBaseButton can geberate menu=-1. This line fixes it and selectes the first item.
  if (menu==-1) 
    menu = 0;
  menuwindow &m = *(pp.p[menu]);
  int item = (menu == pp.menu_number) ? pp.item_number : m.selected;
  while (++item < m.numitems) {
    const MenuItem* m1 = m.menu->next(item);
...
int menuwindow::early_hide_handle(int e) {
  ...
  case FL_SHORTCUT: {
    for (int mymenu = pp.nummenus; mymenu--;) {
      ...
      int item; const MenuItem* m = mw.menu->find_shortcut(&item);
      if (m) {
  ...
  case FL_RELEASE:
	...
	(pp.menubar && pp.current_item && !pp.current_item->submenu()) // button
      ...
      const MenuItem* m = pp.current_item;
      if (m && button && (m->flags & (FL_MENU_TOGGLE|FL_MENU_RADIO))) {
	((MenuBase*)button)->picked(m);
...
const MenuItem* MenuItem::pulldown(
    int X, int Y, int W, int H,
    const MenuItem* initial_item,
    const MenuBase* pbutton,
    const MenuItem* t,
    int menubar) const
{
  ...
  for (;;) {
    ...
    // get events:
    {const MenuItem* oldi = pp.current_item;
    Fl::wait();
  ...
  STARTUP:
    ...
    if (m->submenu()) {
      const MenuItem* title = m;
      const MenuItem* menutable;
      if (m->flags&FL_SUBMENU) menutable = m+1;
      else menutable = (MenuItem*)(m)->user_data_;
...
{
  static MenuItem dummy; // static so it is all zeros
  dummy.text = title;
...
const MenuItem* MenuItem::find_shortcut(int* ip) const {
  const MenuItem* m = first();
...
const MenuItem* MenuItem::test_shortcut() const {
  const MenuItem* m = first();
  const MenuItem* ret = 0;
  if (m) for (; m->text; m = m->next()) {
    if (m->activevisible()) {
      ...
      if (!ret && m->submenu()) {
	const MenuItem* s =
	  (m->flags&FL_SUBMENU) ? m+1:(const MenuItem*)m->user_data_;
	ret = s->test_shortcut();
      }
    }
  }
