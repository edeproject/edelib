...
// Methods to alter the menu in an MenuBase widget.
...
static MenuItem* insert(
  ...
  if (array == local_array && size >= local_array_alloc) {
    ...
    MenuItem* newarray = new MenuItem[local_array_alloc];
    memmove(newarray, array, size*sizeof(MenuItem));
    delete[] local_array;
    local_array = array = newarray;
  }
  // move all the later items:
  memmove(array+n+1, array+n, sizeof(MenuItem)*(size-n));
  // create the new item:
  MenuItem* m = array+n;
...
int MenuBase::add(const char *t, int s, Fl_Callback *c,void *v,int f) {
  ...
  if (this != fl_menu_array_owner) {
	...
	local_array = menu_ = new MenuItem[local_array_alloc];
        memset(local_array, 0, sizeof(MenuItem) * local_array_alloc);
      }
      memset(menu_, 0, sizeof(MenuItem));
      local_array_size = 1;
    }
...
int MenuBase::add(const char *str) {
  char buf[1024];
  int r = 0;
  while (*str) {
    int sc = 0;
...
void MenuBase::replace(int i, const char *str) {
